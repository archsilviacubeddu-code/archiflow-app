...
def mostra_lavori(df, DB_FILE):
    st.markdown("""
        ...
        div.stButton > button[key="btn_back"] {
            height: 30px !important; font-size: 12px !important; margin-bottom: 10px !important;
        }
...
def render_modulo(sezione, df, DB_FILE):
    ...
    with col_lista:
        ...
        # Barra superiore ricerca/aggiungi
        ...
        if st.button("🗑️ CANCELLA", use_container_width=True):
            ids = [k.replace("workcheck_", "") for k, v in st.session_state.items() if k.startswith("workcheck_") and v is True]
            if ids:
    ...
    with col_scheda:
        ...
        if idx is not None and idx in df.index:
            ...
            with st.container(border=True):
                ...
                u_ind = c_c.text_input("🏠 Indirizzo", r['Indirizzo'])
                ...
                with b_agg:
                    ...
                    if st.button("🔄 AGGIORNA", key=f"up_w_{idx}", use_container_width=True):
