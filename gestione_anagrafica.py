...
def mostra_lavori(df, DB_FILE):
    st.markdown("""
        ...
        /* LISTA SINISTRA - CLONATA DA ANAGRAFICA */
        ...
        /* BOTTONI AZIONE SUPERIORI - CLONATI DA ANAGRAFICA */
        ...
        /* BOTTONI SCHEDA - CLONATI DA ANAGRAFICA */
...
def render_modulo_lavoro(sezione, df, DB_FILE):
    ...
    with col_lista:
        ...
        # Barra superiore ricerca/aggiungi [3, 1]
        ...
        if st.button("🗑️ CANCELLA", use_container_width=True):
            selezionati = [k.replace("workcheck_", "") for k, v in st.session_state.items() if k.startswith("workcheck_") and v is True]
            if selezionati:
        ...
        for i, r in df_filt.iterrows():
            ...
            c_sel.checkbox("", key=f"workcheck_{r['id']}", label_visibility="collapsed")
            # Lista Nome — Lavoro
    ...
    with col_scheda:
        ...
        if idx is not None and idx in df.index:
            ...
            # Corpo scheda identico ad Anagrafica
            ...
            with b_agg_col:
                ...
                if st.button("🔄 AGGIORNA", key=f"up_work_{idx}", use_container_width=True):
            ...
            with b_del_col:
                ...
                if st.button("🗑️ ELIMINA", key=f"del_work_{idx}", use_container_width=True):
                    df = df.drop(idx)
                    df.to_csv(DB_FILE, index=False)
                    st.session_state.lavoro_sel = None
                    st.rerun()
