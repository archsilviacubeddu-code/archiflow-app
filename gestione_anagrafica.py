...
def mostra_anagrafica(conn):
    # CSS: ALLINEAMENTO BARRA SUPERIORE E BOTTONI FONDO
    st.markdown("""
        <style>
        /* Allinea perfettamente la barra superiore (Cerca, Aggiungi, Cancella) */
        div[data-testid="stColumn"] {
            display: flex !important;
            align-items: flex-end !important;
        }
        
        /* Bottoni lista nomi a sinistra */
    ...
    with col_scheda:
        ...
        if sel_id:
            ...
            u_cli = c_f1.text_input("Ragione Sociale", r['Cliente'])
            u_cf = c_f2.text_input("C.F. / P.IVA", r['CF_PIVA'])
            
            c_f3, c_f4, c_f5 = st.columns([1.5, 1, 1.5])
            ...
            u_pra = c_f3.selectbox("Pratica", lista_p, index=lista_p.index(r['Pratica']) if r['Pratica'] in lista_p else 14)
            u_sta = c_f4.selectbox("Stato", ["Da fare", "In corso", "Annullata", "Conclusa", "Sospesa"], index=0)
            u_sca = c_f5.text_input("📅 Scadenza", r['Scadenza'])
            ...
            with b2:
                ...
                if st.button(f"📋 CHECKLIST {u_pra.upper()}", use_container_width=True):
                    apri_dialog_checklist(conn, sel_id, u_pra, r['docs_json'])
                st.markdown('</div>', unsafe_allow_html=True)
        else:
            st.info("👈 Seleziona un cliente.")

# 3. FINESTRA DIALOGO PER LA CHECKLIST
@st.dialog("📋 Checklist Documenti")
def apri_dialog_checklist(conn, sel_id, pratica, docs_json):
