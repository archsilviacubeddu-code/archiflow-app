...
def mostra_anagrafica(conn):
    # --- 1. CSS: STILE E ALLINEAMENTO ---
    st.markdown("""
        <style>
        /* Allinea verticalmente Cerca, Aggiungi e Cancella */
            ...
            if not res.empty:
                ...
                with b_col2:
                    ...
                    if st.button("📋 CHECKLIST", use_container_width=True):
                        apri_checklist_dialog(conn, sel_id, u_pra, r['docs_json'])
                    st.markdown('</div>', unsafe_allow_html=True)
        else:
            st.info("👈 Seleziona un cliente dalla lista.")

# --- 4. FUNZIONE DIALOG PER CHECKLIST ---
@st.dialog("Gestione Checklist Documenti")
def apri_checklist_dialog(conn, sel_id, pratica, docs_json):
    st.write(f"Documenti necessari per: **{pratica}**")
