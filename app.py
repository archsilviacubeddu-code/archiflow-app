...
def genera_verbale_ai(testo_appunti, api_key):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "Sei l'Architetto Silvia Cubeddu. Trasforma gli appunti di cantiere in un verbale di sopralluogo professionale, formale e strutturato. Usa termini tecnici appropriati (finiture, conformità, opere murarie)."},
    ...
    if st.session_state.get("sotto_menu") == "DL":
        if st.session_state.get("cantiere_aperto"):
            ...
            with t1:
                ...
                nuovo_stato = st.selectbox("Stato", ["Iniziale", "In Corso", "Sospeso", "Ultimato"], index=["Iniziale", "In Corso", "Sospeso", "Ultimato"].index(row['Stato']))
            ...
            with t3:
                st.subheader("🤖 Verbale AI")
                st.info("Scrivi o incolla qui i tuoi appunti presi a voce o via testo.")
                appunti = st.text_area("Appunti di cantiere:", value=row['Note'], height=150)
                if st.button("🤖 GENERA VERBALE PROFESSIONALE"):
                ...
                if "verbale_generato" in st.session_state:
                    appunti = st.text_area("Verbale Finale (Modificabile):", value=st.session_state.verbale_generato, height=300)
            ...
            if st.button("💾 SALVA CANTIERE", use_container_width=True):
            ...
            with st.expander("➕ NUOVO CANTIERE"):
                with st.form("n_c"):
                    ...
                    tip = st.radio("Tipo", ["Interni", "Esterni"], horizontal=True)
                    if st.form_submit_button("CREA"):
                        new_c = {"id_cantiere": str(len(df_can)+1), "Cliente": cli, "Indirizzo": ind, "Tipo": tip, "Stato": "Iniziale", "Progresso": "0"}
                        df_can = pd.concat([df_can, pd.DataFrame([new_c])], ignore_index=True)
                        salva_db(df_can, DB_CAN); st.rerun()
            
            search = st.text_input("🔍 Cerca...")
            filt = df_can[df_can.apply(lambda r: search.lower() in r.astype(str).str.lower().values, axis=1)] if search else df_can
            for _, r in filt.iterrows():
                with st.container():
                    st.markdown(f'<div class="cantiere-card"><b>{r["Cliente"]}</b> - {r["Indirizzo"]} ({r["Stato"]} - {r["Progresso"]}%)</div>', unsafe_allow_html=True)
