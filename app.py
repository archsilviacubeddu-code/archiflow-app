...
st.markdown("""
    ...
    /* --- TASTI TESTUALI TRIPLICATI PER DL --- */
    ...
    /* Colori testi per i tasti della tabella */
    div[data-testid="column"] button[key^="up_"] p { color: #2A9D8F !important; }
    div[data-testid="column"] button[key^="op_"] p { color: #457B9D !important; }
    div[data-testid="column"] button[key^="del_"] p { color: #E63946 !important; }
...
if menu == "HOME":
    ...
    with col_list:
        if not df_filt_ana.empty:
            ...
            if selected_ids and st.button(f"📂 APRI SCHEDA CLIENTE", use_container_width=True):
    ...
    with col_form:
        ...
        if modo:
            ...
            with st.form("form_ana"):
                ...
                st.write("---")
                if st.form_submit_button("✅ AGGIORNA / SALVA", use_container_width=True):
    ...
    if st.session_state.get("sotto_menu") == "DL":
        ...
        if st.session_state.get("c_aperto"):
            ...
            st.markdown('<div class="table-header">ELENCO CANTIERI</div>', unsafe_allow_html=True)
        ...
        with c1:
            ...
            if st.button("📐\nRILIEVI", key="ril_main", use_container_width=True): st.toast("Modulo Rilievi"); st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with c2:
            st.markdown('<div class="btn-pra">', unsafe_allow_html=True)
            if st.button("📋\nPRATICHE", key="pra_main", use_container_width=True): st.toast("Modulo Pratiche"); st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown('<div class="btn-mill">', unsafe_allow_html=True)
            if st.button("📊\nMILLESIMI", key="mill_main", use_container_width=True): st.toast("Modulo Millesimi"); st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with c3:
            st.markdown('<div class="btn-ape">', unsafe_allow_html=True)
            if st.button("⚡\nAPE", key="ape_main", use_container_width=True): st.toast("Modulo APE"); st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
