...
st.markdown("""
    ...
    /* --- FORZATURA TASTI DIREZIONE LAVORI TRIPLICATI --- */
...
def genera_verbale_ai(testo, key):
    ...
    try:
        ...
        res = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": "Sei l'Architetto Silvia Cubeddu. Trasforma gli appunti in verbali tecnici professionali."},
    ...
    if st.session_state.get("sotto_menu") == "DL":
        ...
        if st.session_state.get("c_aperto"):
            ...
            with tab2:
                ...
                if "v_res" in st.session_state: st.text_area("Verbale AI:", value=st.session_state.v_res, height=200)
            ...
            if st.button("➕ AGGIUNGI NUOVO CANTIERE"):
                new_c = {"id_cantiere": str(len(df_can)+1), "Cliente": "Nuovo", "Indirizzo": "-", "Tipo": "Interni", "Stato": "Da iniziare", "Note": "", "chk_durc": "0", "chk_urbanistica": "0", "chk_finelavori": "0"}
            ...
            for i, r in df_can.iterrows():
                col1, col2, col3, col4, col5 = st.columns([1.5, 2, 1, 1, 3.5])
