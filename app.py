...
# CSS AGGRESSIVO: Pulizia totale, font grandi, zero ID
st.markdown("""
    <style>
    ...
    .bg-red { background-color: #ef4444; box-shadow: 0 0 15px rgba(239, 68, 68, 0.6); }
    .bg-yellow { background-color: #f59e0b; box-shadow: 0 0 15px rgba(245, 158, 11, 0.6); }
    .bg-green { background-color: #10b981; box-shadow: 0 0 15px rgba(16, 185, 129, 0.6); }
    </style>
...
if st.session_state.menu_sel == "HOME":
    ...
    with col1: # CARD SCADENZE
        ...
        if not df_scad.empty:
            for _, r in df_scad.sort_values(by="Scadenza").iterrows():
                stato = r['Stato'].lower()
                dot_color = "bg-red"
    ...
    with col2: # CARD ULTIMI INSERIMENTI (Senza ID)
        st.markdown('<div class="card-home"><h3>🆕 Ultimi Lavori</h3>', unsafe_allow_html=True)
        if not df_globale.empty:
            # Prendiamo gli ultimi 6 e invertiamo l'ordine per avere il più nuovo in alto
            for _, r in df_globale.tail(6).iloc[::-1].iterrows():
                if r['Cliente'].strip() != "": # Non mostriamo quelli senza nome
                    st.markdown(f"""
                        <div class="item-row">
                            <div class="info-box">
                                <div>
